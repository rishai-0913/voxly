...
async def insert_note(data: dict) -> dict:
...
async def list_notes() -> list[dict]:
    cursor = notes_col().find({}).sort("created_at", -1)
    return [_serialize(d) async for d in cursor]


async def find_note(note_id: str) -> dict | None:
    doc = await notes_col().find_one({"_id": ObjectId(note_id)})
    return _serialize(doc) if doc else None


async def update_note(note_id: str, data: dict) -> dict | None:
    await notes_col().update_one({"_id": ObjectId(note_id)}, {"$set": data})
    return await find_note(note_id)


async def remove_note(note_id: str) -> bool:
